...
<div class="page-title-box">
    <div class="row align-items-center">
        <div class="col-md-8">
            <h6 class="page-title">Events {{ $event->title }}</h6>
            <ol class="breadcrumb m-0">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">Beranda</a></li>
                <li class="breadcrumb-item active" aria-current="Events {{ $event->title }}">Events {{ $event->title }}</li>
            </ol>
        </div>
        <div class="col-md-4">
...
<script>
    var table = $('#datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('events.detailRegister',['id' => $event->id]) }}",
        columns: [
            {
                data: 'kode_tiket',
                name: 'kode_tiket'
