...
<script src="{{ asset('backend/assets2/js/iziToast.min.js') }}"></script>
    ...
    <script>
        ...
        function edit(id) {
            $.ajax({
                type: 'GET',
                url: "{{ url('b/roles') }}"+'/'+id+'/edit',
                contentType: "application/json;  charset=utf-8",
                cache: false,
                success: function(result){
                    document.getElementById('edit_title').innerHTML = result.roles.role;
                    $('#edit_id').val(result.roles.id);
                    $('#edit_role').val(result.roles.role);
