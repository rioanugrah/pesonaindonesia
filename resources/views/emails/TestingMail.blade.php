...
<head>
    ...
    <title>Invoice - {{ $invoice->transaction_code }}</title>
...
// dd($name[0]->first_name);
    $transaction_detail = json_decode($invoice->transaction_order);
                ...
                <div class="tm_invoice_in">
                        ...
                        <div class="tm_invoice_info_left">
                            ...
                            <p>
                                <b class="tm_f16 tm_primary_color"
                                    style="font-size: 10pt">{{ $transaction_detail->first_name.' '.$transaction_detail->last_name }}</b> <br>
                                {{ $transaction_detail->address . ' - ' . $transaction_detail->phone }} <br>
                            ...
                            <div class="tm_grid_row tm_col_3 tm_invoice_info_in tm_accent_bg">
                                <div>
                                    ...
                                    <b class="tm_f16 tm_white_color">Rp.
                                        ...
                                        {{ number_format($invoice->transaction_price, 0, ',', '.') }}</b>
                                </div>
                                <div>
                                    <span class="tm_white_color_60">Invoice Date:</span> <br>
                                    <b
                                        class="tm_f16 tm_white_color">{{ \Carbon\Carbon::create($invoice->created_at)->isoFormat('LL') }}
                                    </b>
                                </div>
                                <div>
                                    <span class="tm_white_color_60">Invoice No:</span> <br>
                                    {{-- <b class="tm_f16 tm_white_color">{{ $details['invoice'] }}</b> --}}
                                    <b class="tm_f16 tm_white_color">{{ $invoice->transaction_code }}</b>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="tm_round_border">
                            <div class="tm_table_responsive">
                                <table>
                                    ...
                                    <tbody>
                                        @if (empty($transaction_detail->item_details))
                                        <td class="tm_width_7">
                                            <p class="tm_m0 tm_f16 tm_primary_color" style="font-size:10pt">
                                                {{ $invoice->transaction_unit }}
                                            </p>
                                        </td>
                                        <td class="tm_width_2">Rp.
                                            {{ number_format($invoice->transaction_price, 0, ',', '.') }}
                                        </td>
                                        <td class="tm_width_1">{{ $invoice->transaction_qty }}</td>
                                        <td class="tm_width_2 tm_text_right">Rp.
                                            {{ number_format($invoice->transaction_price * $invoice->transaction_qty, 0, ',', '.') }}
                                        </td>
                                        @else
                                        @endif
                                    </tbody>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td
                                                ...
                                                Rp. {{ number_format($invoice->transaction_price, 0, ',', '.') }}</td>
                                            {{-- <td
                                                class="tm_width_3 tm_primary_color tm_text_right tm_border_none tm_bold">
                    ...
                    <div class="tm_note tm_font_style_normal tm_text_center">
                        ...
                        <p class="tm_m0" style="font-style: italic">Terakhir diupdate
                            {{ \Carbon\Carbon::parse($invoice->updated_at)->isoFormat('LLL') }}</p>
                    </div><!-- .tm_note -->
                </div>
            </div>
